...
TEST_CASE("Test the constructors of Token", "[token]") {
    ...
    const model::Token* token1 {new model::Token{0, 0, "token1"}};
    const model::Token* token2 {new model::Token{1, 0, value}};
...
TEST_CASE("Test the getter of Token", "[token]") {
    const model::Token token{0, 0, "token1"};
